...
import type { ExportSettings, MeshNode, DocMesh, HtmlFile } from '../types';
...
interface RightSidebarProps {
  ...
  savedMeshes?: DocMesh[];
  onNodeMetadataUpdate?: (nodeId: string, updates: Partial<MeshNode>) => void;
  onMeshPreviewModeChange?: (mode: 'node' | 'index') => void;
  onNewMesh?: () => void;
...
export function RightSidebar({
  ...
  // Get selected node if in DocMesh mode
  const selectedNode = selectedNodeId && currentMesh
    ? currentMesh.nodes.get(selectedNodeId)
    : null;

  // Get HTML file for selected node to get default title
  const htmlFile = selectedNode
    ? htmlFiles.get(selectedNode.htmlFileId)
    : null;

  return (
    <aside className="w-full md:w-80 border-l border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-900 flex flex-col overflow-y-auto transition-colors duration-300">
      <div className="p-6 space-y-6">
        {/* DocMesh Settings Section - only show in docmesh mode */}
        {appMode === 'docmesh' && (
          <>
            {/* Mesh Controls */}
            {onNewMesh && onSaveMesh && onLoadMesh && onExportMesh && onDeleteMesh && onDeleteAllMeshes && (
              <DocMeshControls
                currentMesh={currentMesh}
                savedMeshes={savedMeshes}
                onNewMesh={onNewMesh}
                onSaveMesh={onSaveMesh}
                onLoadMesh={onLoadMesh}
                onExportMesh={onExportMesh}
                onDeleteMesh={onDeleteMesh}
                onDeleteAllMeshes={onDeleteAllMeshes}
                onMeshNameUpdate={onMeshNameUpdate}
              />
            )}
            ...
            <div>
              ...
              <div className="mb-4">
                <label className="flex items-center gap-2 text-sm font-medium text-slate-700 dark:text-slate-300 mb-2 transition-colors duration-300">
                  Preview Mode
                  <Tooltip content="Choose between viewing individual nodes or the complete index document">
                    <button
                      className="text-slate-400 hover:text-slate-600 dark:hover:text-slate-300 transition-colors duration-200"
                      aria-label="Preview mode info"
                    >
            ...
            {/* Node Metadata Editor - only show when a node is selected */}
            {selectedNode && (
              <div className="pt-6 border-t border-slate-200 dark:border-slate-800 transition-colors duration-300">
                ...
                <div className="mb-4">
                  <label className="flex items-center gap-2 text-sm font-medium text-slate-700 dark:text-slate-300 mb-2 transition-colors duration-300">
                    Display Title
                    <Tooltip content="Custom title for navigation links. Leave empty to use filename.">
                      <button
                        className="text-slate-400 hover:text-slate-600 dark:hover:text-slate-300 transition-colors duration-200"
                        aria-label="Display title info"
                      >
                        <Info className="w-4 h-4" />
                      </button>
                    </Tooltip>
                  ...
                  <input
                    ...
                    onChange={(e) => onNodeMetadataUpdate?.(selectedNode.id, { title: e.target.value })}
                    placeholder={htmlFile?.name || 'Enter title...'}
                    className="w-full px-3 py-2 rounded-lg border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-900 dark:text-slate-100 placeholder-slate-400 dark:placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-indigo-500 text-sm transition-colors duration-300"
                  />
                  {!selectedNode.title && htmlFile && (
                    <p className="mt-1 text-xs text-slate-500 dark:text-slate-400 transition-colors duration-300">
                      Default: {htmlFile.name}
                    </p>
                  )}
                </div>
                ...
                <div className="mb-4">
                  <label className="flex items-center gap-2 text-sm font-medium text-slate-700 dark:text-slate-300 mb-2 transition-colors duration-300">
                    Description
                    <Tooltip content="Optional description shown in navigation">
                      <button
                        className="text-slate-400 hover:text-slate-600 dark:hover:text-slate-300 transition-colors duration-200"
                  ...
                  <textarea
                    ...
                    onChange={(e) => onNodeMetadataUpdate?.(selectedNode.id, { description: e.target.value })}
                    placeholder="Enter description..."
                    rows={3}
                    className="w-full px-3 py-2 rounded-lg border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-900 dark:text-slate-100 placeholder-slate-400 dark:placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-indigo-500 text-sm transition-colors duration-300 resize-none"
                  />
                </div>
              </div>
            )}
            ...
            <div className="pt-6 border-t border-slate-200 dark:border-slate-800 transition-colors duration-300">
              ...
              <div className="space-y-3">
                <p className="text-sm text-slate-600 dark:text-slate-400 transition-colors duration-300">
                  Export settings from the markdown mode will be applied to the generated index document.
                </p>
              </div>
            </div>
          </>
        ...
        {/* Export Settings Section - only show in markdown mode */}
        {appMode === 'markdown' && (
          <div>
            <h2 className="text-lg font-semibold text-slate-900 dark:text-slate-100 mb-4 transition-colors duration-300">
              Export Settings
        ...
        {/* Status Card - shown in both modes */}
        <div className="pt-6 border-t border-slate-200 dark:border-slate-800 transition-colors duration-300">
